...
if [[ $ACTION == add ]]; then
  for host in "${hosts[@]}"; do
    sudo ufw allow from "$host" to any port "$PORT" proto tcp comment OmaSonos
  done
  echo "Allowed TCP $PORT from ${#hosts[@]} discovered Sonos speaker address(es)."
else
  for host in "${hosts[@]}"; do
    sudo ufw --force delete allow from "$host" to any port "$PORT" proto tcp
  done
